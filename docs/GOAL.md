...
**Serving upgrade:** Hugging Face Accelerate for low-memory placement and vLLM for the planned multi-user serving path  
**Quantization policy:** User-selected; default loading preserves the checkpoint representation published by the configured model source, while explicit loading modes may request a supported quantization backend or format  
**First validated model path:** Mixtral 8x7B-Instruct-v0.1 (or a compatible local Mixtral checkpoint) for architecture-specific expert paging
**General model path:** Any Hugging Face-compatible open-source causal language model
...
## User-controlled loading and quantization

Model loading must preserve user intent rather than impose a project-wide precision
policy:

- In the default `quantization="auto"` mode, do not quantize or dequantize at
  runtime; load the representation supplied by the configured Hugging Face or
  local checkpoint, including checkpoint-declared quantization when supported.
- If the user requests full published weights, load those weights as provided;
  do not replace them with a project-selected quantization format.
- If the user explicitly selects a supported quantization backend or format,
  apply it during loading and report the effective dtype, backend, and format.
- If the requested mode is unavailable or incompatible with the model or device,
  fail clearly. Never silently switch to another precision or quantization mode.

The available modes are runtime and backend capabilities, not hard-coded model
rules. Quantization metadata, loading mode, and effective placement must be
visible in diagnostics and benchmark results.
